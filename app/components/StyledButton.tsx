...
export function StyledButton({ children, icon: Icon, onClick, type = "button", className = "" }: StyledButtonProps) {
  return (
    <motion.button
      ...
      onClick={onClick}
      whileHover={{ x: 10 }}
      className={`group flex items-center gap-3 text-white border border-white/20 px-8 py-4 hover:border-[var(--accent-primary)] bg-white/10 hover:bg-[var(--accent-primary)]/10 transition-all duration-300 cursor-pointer ${className}`}
    >
